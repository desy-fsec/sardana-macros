...
class aNscanCheck(Hookable):
    
    hints = { 'scan' : 'aNscanCheck', 'allowsHooks': ('pre-scan', 'pre-move', 'post-move', 'pre-acq', 'post-acq', 'post-step', 'post-scan') }
    #env = ('ActiveMntGrp',)
    
    """N-dimensional scan. This is **not** meant to be called by the user,
    but as a generic base to construct ascan_checkabs, a2scan_checkabs, a3scan_checkabs,..."""
...
class dNscanCheck(aNscanCheck):
    ...
    hints = copy.deepcopy(aNscanCheck.hints)
    hints['scan'] = 'dNscanCheck'
    ...
    def _prepare(self, motorlist, startlist, endlist, scan_length, integ_time, mode=StepMode, **opts):
        ...
        aNscanCheck._prepare(self, motorlist, starts, finals, scan_length, integ_time, mode=mode, **opts)

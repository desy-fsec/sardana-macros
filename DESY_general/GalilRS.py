...
class GalilRS(Macro):
    ...
    param_def = [['opMode', Type.String, None, 
                  '-x execute, -f forced, execute ignoring the current positions']]
    ...
    def run( self, opMode):
        ...
        try:
            galilCtrlProxy = PyTango.DeviceProxy( galilCtrl[0])
                ...
                self.output("GalilRS failed to create proxy to %s" % galilCtrl[0])
                return False
            ...
            if opMode != '-f':
                ...
                while retryCount < 2:
                    try:
                        ...
                        self.output("GalilRS failed to read Position of %s" % elm)
                        retryCount += 1
                else:
                    self.output( "GalilRS: tried to read %s position %d times, failed" % elm)
                    self.output( "GalilRS: consider to use '-f' option")
                    return False
                self.output( "GalilRS: %s is at %g (old position)" % ( elm, dct[ elm][ 'position']))
        #
        ...
        while 1:
            ...
            for elm in galilMotors:
                ...
                galilCtrlProxy.ReadAllRemaining()
                self.output( "GalilRS: %s is at %g (target: abs(pos) == %g)" % (elm, pos, posPowerOn))
        ...
        if opMode != '-f':
            ...
            for elm in galilMotors:
                ...
                self.output( "GalilRS: started %s to %g" % ( elm, dct[ elm][ 'position']))
                while 1:
                    pos = dct[elm]['proxy'].Position
                    self.output( "GalilRS: %s is at %g (target: %g) " % (elm, pos,dct[elm]['position']))
                    if math.fabs( math.fabs( dct[elm]['position']) - math.fabs( pos)) < 0.1:
                        break
                    time.sleep(1)

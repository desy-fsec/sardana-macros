...
class set_harmonic(Macro):
    ...
    def run(self, harmonic):
        ...
        try:
            ...
            self.error("Not able to create proxy to undulator device %s." % undulator_device_name)
            self.info("\nSet the name of the Pool undulator device in the Spock environment variable UndulatorDevice, ex.:\n\n   senv UndulatorDevice undulator")
            return

        undulator_tango_device = PyTango.DeviceProxy(undulator_device.TangoDevice)

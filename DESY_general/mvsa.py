...
class mvsa(Macro):
    ...
    Used environment variables:
      ...
      ScanHistory                -> motor name and scan type,
                                    supported: ascan, a2scan, a3scan, dscan,
                                               d2scan, d3scan, hscan, kscan,
                                               lscan, hklscan
...
"""
    param_def = [
        ['mode', Type.String, 'peak',
         "Options: 'show','peak','cms','cen','dip','dipm','dipc','slit', "
         "'slitm', 'slitc', 'step','stepm' and 'stepc', 'stepssa', "
         "'stepcssa', 'stepmssa'"],
        ['interactiveFlag', Type.Integer, 1,
         " '1' query before move (def.) "],
    ]
    result_def = [["result", Type.String, None,
    ...
    def run(self, mode, interactiveFlag):
        ...
        if toMonitorFunc is not None:
            ...
            if hsh['result'].upper() != 'DONE':
                self.output(
                    "mvsa: SardanaMonitor did not send DONE, instead: %s"
                    % hsh['result'])
                return result
            if len(hsh['getData'].keys()) == 0:
                self.output("mvsa: no data")
                return result
            if signalCounter.upper() is not hsh['getData'].keys():
                self.output(
                    "mvsa: column %s is missing (from SM)" % signalCounter)
                return result
            flagDataFound = True
            message, xpos, xpeak, xcms, xcen = calc.fastscananalysis(
            ...
            for col in a.columns:
                if col.name == signalCounter:
                    ...
                    if mode.lower() == 'show':
                        ...
                        #
                        ssaDct = calc.ssa(
                            np.array(col.x), np.array(col.y), False)
                    flagDataFound = True
                    break

        if not flagDataFound:
            self.output(
                "Column %s not found in %s" % (signalCounter, fileName))
            for col in a.columns:
        ...
        if mode.lower() == 'show':
            ...
            self.output("mvsa: xpeak %g, cms %g cen  %g"
                        % (xpeak, xcms, xcen))
            self.output(
                "mvsa: status %d, reason %d"
                % (ssaDct['status'], ssaDct['reason']))
            self.output(
                "mvsa: xpeak %g, cms %g midp %g"
                % (ssaDct['peak_x'], ssaDct['cms'], ssaDct['midpoint']))
            self.output(
                "mvsa: l_back %g, r_back %g"
        ...
        if len(motorArr) == 1:
            ...
            motorArr[1]['targetPos'] = (
                motorArr[1]['stop'] - motorArr[1]['start']) * r + \
                motorArr[1]['start']
        elif len(motorArr) == 3:
            motorArr[0]['targetPos'] = xpos
            motorArr[1]['targetPos'] = (
            ...
            motorArr[2]['targetPos'] = (
                motorArr[2]['stop'] - motorArr[2]['start']) * r + \
                motorArr[2]['start']
        else:
            return result
        #
        ...
        if interactiveFlag == 1:
            ...
            # posStart =
            motorArr[0]['proxy'].position
            #
            # move the red arrow to the target position
            #
            if isPysp:
                toMonitorFunc(
                    {'command': ['display %s' % signalCounter,
        ...
        while moving:
            ...
            # if isPysp:
            #    toMonitorFunc(
            #        {'command': ['setArrowCurrent %s position %g' % \
        ...
        #
        if isPysp:
            toMonitorFunc({'command':
                           ['setArrowMisc %s hide' % signalCounter]})
        for elm in (motorArr):
            p = PyTango.DeviceProxy(elm['name'])

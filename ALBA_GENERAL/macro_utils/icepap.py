import time
import PyTango
from sardana.macroserver.macro import Table
...
def home(macro, motorsInfoList):
    ...
    HM_CMD, HM_STATUS_CMD, HM_POS_CMD, HM_ENCIN_CMD, ABORT_CMD = \
        populate_homing_commands(motors, hmDirections, group=False,
                                 strict=False)
...
def home_group(macro, motorsInfoList):
    ...
    macro.debug('1')
    HM_CMD, HM_STATUS_CMD, HM_POS_CMD, HM_ENCIN_CMD, ABORT_CMD = \
        populate_homing_commands(motors, hmDirections, group=True,
                                 strict=False)
    macro.debug('2')
    macro.debug('HM_CMD: %s', HM_CMD)
    macro.debug('HM_STATUS_CMD: %s', HM_STATUS_CMD)
    ...
    try:
        while True:
            ...
            for i, motInfo in enumerate(motorsInfoList):
                ...
                motInfo['position'] = motor_pos
                macro.debug('Motor: %s, position: %s',
                            motor.alias(), motInfo['position'])
                homingStatus = homeStats[i]
                motInfo['status'] = homingStatus
                if homingStatus == 'FOUND':
                    motInfo['homed'] = True
...
def home_strict(macro, motorsInfoList):
    ...
    :return: (boolean) True when first motors finds home, False in all other
                       cases
    """
    ...
    hmDirections = [i.get('direction') for i in motorsInfoList]
    HM_CMD, HM_STATUS_CMD, HM_POS_CMD, HM_ENCIN_CMD, ABORT_CMD = \
        populate_homing_commands(motors, hmDirections, group=True, strict=True)
    macro.debug('HM_CMD: %s', HM_CMD)
    macro.debug('HM_STATUS_CMD: %s', HM_STATUS_CMD)
    macro.debug('HM_POS_CMD: %s', HM_POS_CMD)
    macro.debug('HM_ENCIN_CMD: %s', HM_ENCIN_CMD)

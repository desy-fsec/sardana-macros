...
class qzscan(Macro):
    ...
    hints = { 'scan' : 'qzscan', 'allowsHooks': ('pre-scan', 'pre-move', 'post-move', 'pre-acq', 'post-acq', 'post-step', 'post-scan') }

    param_def = [
        ['sample0',      Type.Float, None, 'sample motor position for example energy'],
        ['detector0',    Type.Float, None, 'detector motor position for example energy'],
    ...
    def prepare(self, sample0, detector0, energy0, energy_start, energy_end, intervals, integ_time, **opts):
        ...
        try:
            motor_energy_name = environment['%s%s' % (env_prefix, 'motor_energy_name')]
            motor_energyid_name = environment['%s%s' % (env_prefix, 'motor_energyid_name')]
            motor_sample_name = environment['%s%s' % (env_prefix, 'motor_sample_name')]
            motor_detector_name = environment['%s%s' % (env_prefix, 'motor_detector_name')]
        except KeyError, e:
            msg = 'Unable to recover motor names from environment. Please define %s* environment variables' % env_prefix
            raise ScanException({ 'msg' : msg })
        except Exception, e:
            msg = 'Unexpected exception while trying to recover motor names from environment:\n%s' % str(e)
            raise ScanException({ 'msg' : msg })
        ...
        try:
            ...
            moveables = [motor_energy, motor_energyid, motor_sample, motor_detector]
        except Exception, e:
            msg = 'Unable to access motors %s and/or %s' % (motor_sample_name, motor_detector_name)
            self.error(msg)
            raise ScanException({ 'msg' : msg })
    ...
    def _generator(self):
        ...
        point_id = 0
        energies = numpy.linspace(self.energy_start, self.energy_end, self.intervals+1)
        if 0.0 in energies:
        ...
        for energy in energies:
            ...
            step['positions'] = [energy, energy, sample_position, detector_position]
    ...
    def get_sample_at_qfix(self, energy):
        ...
        """
        return 180.0/numpy.pi * numpy.arcsin((self.energy0/energy)*numpy.sin(self.sample0*numpy.pi/180.0))
    ...
    def get_detector_at_qfix(self, energy):
        ...
        """
        return 180.0/numpy.pi * numpy.arcsin((self.energy0/energy)*numpy.sin(self.sample0*numpy.pi/180.0)) + \
               180.0/numpy.pi * numpy.arcsin((self.energy0/energy) * numpy.sin(numpy.pi/180.0*(self.detector0-self.sample0)))
...
class qxqzscan(qzscan):
    ...
    hints = { 'scan' : 'qxqzscan', 'allowsHooks': ('pre-scan', 'pre-move', 'post-move', 'pre-acq', 'post-acq', 'post-step', 'post-scan') }
    ...
    HC = 4.1357e-15 * 2.997925e17 # h*c (h = 4.1357e-15 eV*s, c = 2.997925e17 nm/s)

    def prepare(self, sample0, detector0, energy0, energy_start, energy_end, intervals, integ_time, **opts):
        super(qxqzscan, self).prepare(sample0, detector0, energy0, energy_start, energy_end, intervals, integ_time, **opts)
    ...
    def build_scan(self,moveables,opts):
        ...
        self.qx_fixed = k0 * (numpy.cos(numpy.pi/180.0*(self.detector0-self.sample0)) - numpy.cos(numpy.pi/180.0*self.sample0))
        self.qz_fixed = k0 * (numpy.sin(numpy.pi/180.0*(self.detector0-self.sample0)) + numpy.sin(numpy.pi/180.0*self.sample0))
        #build scan
        generator=self._generator
        env=opts.get('env',{})
        constrains=[]
    ...
    def get_sample_at_qfix(self, energy):
        ...
        k0 = 2 * numpy.pi / wave_length
        return 0.5 * (numpy.pi/180.0 * numpy.arccos(1.0-0.5*(numpy.power(self.qx_fixed,2)+numpy.power(self.qz_fixed,2))/numpy.power(k0, 2) ) + self.offset)
    ...
    def get_detector_at_qfix(self, energy):
        ...
        wave_length = self.HC / energy
        k0 = 2 * numpy.pi / wave_length
        return 180.0/numpy.pi * numpy.arccos(1.0-0.5*(numpy.power(self.qx_fixed,2)+numpy.power(self.qz_fixed,2))/numpy.power(k0, 2) )
